...
export type DashboardTab = 'dashboard' | 'calendar' | 'appointments' | 'customers' | 'settings' | 'leads' | 'tasks' | 'reports';
...
export const DashboardSidebar: React.FC<DashboardSidebarProps> = ({
  ...
  const navItems = [
    ...
    ,{ id: 'leads' as DashboardTab, label: 'Leads', icon: Users, badge: undefined },
  ...
  return (
    <aside 
      id="crm-sidebar"
      className="w-16 sm:w-64 bg-[#1A1C1A] text-[#FDFCFB] flex flex-col justify-between border-r border-[#2D302E] shrink-0 h-screen sticky top-0"
    >
      {/* Brand & Top Header */}
      <div>
        <div className="hidden sm:block p-5 border-b border-[#2D302E]">
        ...
        <div className="px-3 py-4 space-y-1">
          <p className="hidden sm:block px-3 text-[10px] font-semibold uppercase tracking-[0.2em] text-[#8B8D8B] mb-2">
            Main Management
          </p>
          ...
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                {item.badge && (
                  <span className={`hidden sm:inline text-[10px] px-1.5 py-0.5 rounded font-mono font-semibold ${
                    isActive 
      ...
      <div className="p-3 border-t border-[#2D302E] space-y-2">
        <p className="hidden sm:block p-3 text-xs capitalize">Signed in · {role}</p>
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <span className="hidden sm:inline">Public Spa Website</span>
          </div>
